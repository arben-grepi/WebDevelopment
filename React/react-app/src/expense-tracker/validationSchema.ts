...
// Import categories from App to reference in validation
import { categories } from "./components/categories";

// Create the Joi schema that defines validation rules for the form fields
const schema = Joi.object({
  // Validate 'description' as a required string with a minimum length of 3 characters
  description: Joi.string().min(3).required().messages({
    "string.min": "Description must be at least 3 characters long",
    "string.empty": "Description is required",
  }),

  // Validate 'amount' as a required number, ensuring it's under 1000
  amount: Joi.number().max(1000).min(0.1).required().messages({
    "number.base": "Amount must be a number",
    "number.max": "Amount must be less than 1000",
    "number.empty": "Amount is required",
  }),

  // Validate 'category' to ensure that a category is selected
  category: Joi.string()
    .valid(...categories)
    .required()
    .messages({
      "any.only": "You must select a valid category",
